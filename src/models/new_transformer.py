...
class MultiHeadedAttention(nn.Module):
    ...
    def forward(self, query, key, value, b_seq=None, mask=None):
        ...
        # 0. rel pos bias
        if self.brpb:
            rel_pos_bias = torch.stack([layer(seq_len, seq_len)
                                       for layer in self.rpb], -1).repeat(batch_size, 1, 1, 1, 1)
        ...
        if self.battn and self.n_b > 1:  # behavior-specific mutual attention
            ...
            query, key, value = [
                torch.einsum(
                    "bnd, Bdhk, bnB->bhnk", x, self.linear_layers[l],
                    F.one_hot(b_seq, num_classes=self.n_b + 1).float()) for l, x in zip(range(3),
                                                                                        (query, key, value))]
        else:
            # 1) Do all the linear projections in batch from d_model => h x d_k
        ...
        x = self.attention(query, key, value, b_mat=b_mat, rpb=rel_pos_bias, W1=self.W1,
                           alpha1=self.alpha1, W2=self.W2, alpha2=self.alpha2, mask=mask)
...
class TransformerBlock(nn.Module):
    def __init__(self, hidden, attn_heads, feed_forward_hidden, n_b, battn, bpff, brpb, dropout):
        ...
        """
        super().__init__()
        self.attention = MultiHeadedAttention(h=attn_heads, n_b=n_b, battn=battn,
                                              brpb=brpb, d_model=hidden, dropout=dropout)
        self.feed_forward = BehaviorSpecificPFF(
            d_model=hidden, d_ff=feed_forward_hidden, n_b=n_b, bpff=bpff, dropout=dropout)
